...
/**
  ...
  * @Addr		0x21 (Read/Write)
	...
	*	------------------------------------------------------------------
  */
void CTRL_REG(I2C_TypeDef* I2Cx, uint8_t reg, uint8_t data){
	
	err=FALSE;
#ifdef HTS221_DEBUG
	printf("CTRL_REG(%x, %x) I2C_GetFlagStatus(I2Cx, I2C_ISR_BUSY)\n\r", reg, data);
#endif	
	timeOut=undateNum<<1;
...
#ifdef HTS221_DEBUG
	printf("CTRL_REG(%x, %x) I2C_SendData()\n\r", reg, data);
#endif
  /* Send Register address */
  I2C_SendData(I2Cx, reg);

#ifdef HTS221_DEBUG
	printf("CTRL_REG(%x, %x) I2C_GetFlagStatus(I2Cx, I2C_ISR_TCR)\n\r", reg, data);
#endif  
	timeOut=undateNum<<1;
...
#ifdef HTS221_DEBUG
	printf("CTRL_REG(%x, %x) I2C_GetFlagStatus(I2Cx, I2C_ISR_STOPF)\n\r", reg, data);
#endif     
  timeOut=undateNum<<1;
...
bool HTS221_Init(void){
	...
	err=FALSE;
	CTRL_REG(I2C2, REG_CTRL1_ADDR, Bit7|Bit2|Bit0);
	//Bit7:Power On, Bit2:Updated after reading, (Bit1)Bit0:01 1 Hz
	if(err) return !err;
	
	CTRL_REG(I2C2, REG_CTRL2_ADDR, Bit7);
...
uint8_t read_Register(uint8_t REG){
	err=FALSE;
#ifdef HTS221_DEBUG
	printf("read_Register(%x) I2C_GetFlagStatus(I2C2, I2C_ISR_BUSY)\n\r",REG);
#endif   	
	timeOut=undateNum<<1;
...
#ifdef HTS221_DEBUG
	printf("read_Register(%x) I2C_GetFlagStatus(I2C2, I2C_ISR_TXIS)\n\r",REG);
#endif     
  timeOut=undateNum<<1;
...
#ifdef HTS221_DEBUG
	printf("read_Register(%x) I2C_SendData()\n\r", REG);
#endif
  I2C_SendData(I2C2, REG);
  
#ifdef HTS221_DEBUG
	printf("read_Register(%x) I2C_GetFlagStatus(I2C2, I2C_ISR_TC)\n\r",REG);
#endif      
	timeOut=undateNum<<1;
  ...
  I2C_TransferHandling(I2C2, SAD, 1, I2C_AutoEnd_Mode, I2C_Generate_Start_Read);
#ifdef HTS221_DEBUG
	printf("read_Register(%x) I2C_GetFlagStatus(I2C2, I2C_ISR_RXNE)\n\r",REG);
#endif     
	timeOut=undateNum<<1; 
...
#ifdef HTS221_DEBUG
	printf("read_Register(%x) I2C_GetFlagStatus(I2C2, I2C_ISR_STOPF)\n\r",REG);
#endif    
	timeOut=undateNum<<1; 
  while(!I2C_GetFlagStatus(I2C2, I2C_FLAG_STOPF))
	{
...
#ifdef HTS221_DEBUG
	printf("read_Register(%x) success.\n\r",REG);
#endif    	
	return data;
}
...
bool CalibHTS221(void){
	...
	uint8_t T0_T1_msb = read_Register(REG_T1_T0_msb);
	if(err) return !err;
	
	uint8_t T0_degC_x8_H = T0_T1_msb & (Bit1|Bit0);//(read_Register(REG_T1_T0_msb) & (Bit1|Bit0) );
	uint8_t T0_degC_x8_L = read_Register(REG_T0_degC);
	if(err) return !err;
	T0_degC = ((float)((T0_degC_x8_H<<8) | (T0_degC_x8_L)))/8;
	
	uint8_t T1_degC_x8_H = T0_T1_msb & (Bit3|Bit2); //(uint16_t)(read_Register(REG_T1_T0_msb)& 0x0C);
	if(err) return !err;
	//T1_degC_x8_H = T1_degC_x8_H >> 2;
	uint8_t T1_degC_x8_L = (uint16_t)read_Register(REG_T1_degC);
	if(err) return !err;
	T1_degC = ((float)( (T1_degC_x8_H<<6)|(T1_degC_x8_L) ))/8;
	
	/* T0, T1 calculate */
	uint8_t T0_out_H = read_Register(REG_T0_OUT_M);
	if(err) return !err;
	uint8_t T0_out_L = read_Register(REG_T0_OUT_L);
	if(err) return !err;
	T0_out = (int16_t)((T0_out_H << 8) | T0_out_L);
	
	uint8_t T1_out_H = read_Register(REG_T1_OUT_M);
	if(err) return !err;
	uint8_t T1_out_L = read_Register(REG_T1_OUT_L);
	if(err) return !err;
	...
	uint8_t H0_T0_out_H =	read_Register(REG_H0_T0_OUT_M);
	if(err) return !err;
	uint8_t H0_T0_out_L = read_Register(REG_H0_T0_OUT_L);
	if(err) return !err;
	H0_out = (int16_t)((H0_T0_out_H << 8) | (H0_T0_out_L));
	
	uint8_t H1_T0_out_H = read_Register(REG_H1_T0_OUT_M);
	if(err) return !err;
	uint8_t H1_T0_out_L =	read_Register(REG_H1_T0_OUT_L);
	if(err) return !err;
	H1_out = (int16_t)((H1_T0_out_H << 8) | (H1_T0_out_L));
	
	
#ifdef HTS221_DEBUG_DATA
	/* DEBUG variable clare */
	printf("----------HTS221 DEBUG Info-----------\n\r");
	
	printf("Temperature Calibration---------------\n\r");
	printf("T0_degC_H:%02d, T0_degC_L:%02d | T0_degC: %.2f\n\r", T0_degC_x8_H, T0_degC_x8_L, T0_degC);
	printf("T1_degC_H:%02d, T1_degC_L:%02d | T1_degC: %.2f\n\r", T1_degC_x8_H, T1_degC_x8_L, T1_degC);
	printf("T0_out_H:%02d, T0_out_L:%02d | T0_out: %04d\n\r", T0_out_H, T0_out_L, T0_out);
	printf("T1_out_H:%02d, T1_out_L:%02d | T1_out: %04d\n\r", T1_out_H, T1_out_L, T1_out);

	printf("Humidity Calibration------------------\n\r");
	printf("H0_rH: %.2f, H1_rH: %.2f\n\r", H0_rH, H1_rH);
	printf("H0_T0_out_H: %02d, H0_T0_out_L: %02d, H0_out: %02d\n\r", H0_T0_out_H, H0_T0_out_L, H0_out );
	printf("H1_T0_out_H: %02d, H1_T0_out_L: %02d, H1_out: %02d\n\r", H1_T0_out_H, H1_T0_out_L, H1_out );
	printf("---------------------\n\r");
...
float GetTemperature(void){
	...
	uint8_t T_out_M = read_Register(REG_T_OUT_M);
	if(err) 
		return 0xff;
	uint8_t T_out_L = read_Register(REG_T_OUT_L);
	if(err) 
		return 0xff;
	
	int16_t T_out = ((T_out_M << 8) | T_out_L);
	float T_degC = (((float)(T_out - T0_out))/(T1_out - T0_out)) * (T1_degC - T0_degC) + T0_degC;
	
#ifdef HTS221_DEBUG_DATA
	printf("---------------------\n\r");
	printf("T_out_M:%02d, T_out_L:%02d | T_out: %04d\n\r", T_out_M, T_out_L, T_out);
	printf("T_degC:%.6f\n\r", T_degC);
#endif
	return T_degC;
}
...
float GetHumidity(void){
	...
	uint8_t H_out_M = read_Register(REG_H_OUT_M);
	if(err) 
		return 0xff;
	uint8_t H_out_L = read_Register(REG_H_OUT_L);
	if(err) 
		return 0xff;
	
	int16_t H_out = ( ( H_out_M << 8) | H_out_L );
	float H_RH = ((float)(H_out - H1_out)/(H0_out - H1_out)*(H0_rH - H1_rH))+H1_rH ; 
	//float H_RH =  (float) ( ( (H_out*(H0_rH-H1_rH)) + (H1_out*H1_rH) - (H0_out*H0_rH) ) / (H1_out-H0_out) );

#ifdef HTS221_DEBUG_DATA	
	printf("H_out_M:%02d, H_out_L:%02d | H_out: %04d\n\r", H_out_M, H_out_L, H_out);
	printf("H_RH:%.2f\n\r", H_RH);
	printf("---------------------\n\r");
#endif
